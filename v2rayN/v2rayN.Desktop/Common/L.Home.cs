...
public sealed partial class L
{
    partial void RegisterHome()
    {
        ...
        // выбирает. Активный глагол, sentence-case, без восклицаний (9.1).
        //
        // ВРЕМЯ ГЛАГОЛА ИСПРАВЛЕНО, И ЭТО НЕ СТИЛИСТИКА. Здесь стояло «загружаем серверы» —
        // обещание в настоящем времени. Тост показывается ПОСЛЕ того, как загрузка кончилась, так что
        // обещание было неверным даже на успехе; а на отказе (провайдер недоступен) оно было прямой
        // неправдой: не загружаем и не загрузим, экран остаётся первым кадром. Строка называет факт и
        // считает серверы — «Подписка добавлена: 40 серверов».
        Add("Home_SubscriptionAdded", "Подписка добавлена: {0}", "Subscription added: {0}");
        Add("Home_SubscriptionExists", "Подписка обновлена: {0}", "Subscription refreshed: {0}");
        // Отказ, у которого раньше не было ни одной поверхности: строка была видна только в журнале,
        // а журнала в этой оболочке нет. Одно предложение — что произошло и что делать, — и выход
        // «Повторить» рядом с ним (G2).
        // «Сохранена», а не «добавлена»: тот же исход приходит и на ПОВТОРНУЮ вставку уже известной
        // подписки, у которой обновление не удалось, — и там «добавлена» было бы неправдой.
        Add("Home_SubscriptionNoServers", "Подписка сохранена, но серверы не пришли — проверьте связь", "The subscription was saved but no servers came back — check your connection");
